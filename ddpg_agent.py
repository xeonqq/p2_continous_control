...
class DDPG_Agent(object):
    def __init__(self, state_dim, action_dim, seed=42):
        ...
        self._actor_local = ActorNet(action_dim, FCBody(state_dim, (64,128, 64)))
        self._actor_target = ActorNet(action_dim, FCBody(state_dim, (64,128, 64)))
        self._actor_optimizer = optim.Adam(self._actor_local.parameters(), lr=LR_ACTOR)

        self._critic_local = CriticNet(action_dim, FCBody(state_dim, (64,128, 64)))
        self._critic_target = CriticNet(action_dim, FCBody(state_dim, (64, 128,64)))
    ...
    def learn(self, experiences):
        ...
        next_actions = self._actor_target(next_states).detach()
        # target_return = rewards + GAMMA * self._critic_target(next_states, next_actions).detach()
        target_Q = self._critic_target(next_states, next_actions).detach()
